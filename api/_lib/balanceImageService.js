...
import { getBalanceScenePrompt } from '../../shared/balanceImagePrompts.js';

/** Stable per-question image — /images/balance_{id}.webp (no date, one per dilemma) */
function safeId(id) {
  return String(id || '').replace(/[^a-zA-Z0-9_-]/g, '');
}

function cacheFilename(id) {
  return `balance_${safeId(id)}.webp`;
}
...
export function getBalanceImagePublicPath(id) {
  return `/images/${cacheFilename(id)}`;
}

export function getBalanceImageLocalPath(id) {
